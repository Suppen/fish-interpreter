...
class FishProgram {
	 ...
	 */
	get width() {
		return this._grid.reduce((maxLen, line) => Math.max(maxLen, line.length), 0);
	}
	...
	/**
	 * Normalizes the grid, making the rows equal length and filling in nullbytes where there are no values
	 *
	 * @private
	 */
	_normalizeGrid() {
		// Make sure every line is the same length
		this._grid.forEach((line) => {
			while (this.width - line.length) {
				line.push(0);
			}
		});

		// Replace undefined with zeros
	...
	advance() {
		...
		try {
			...
			const val = this._grid[this._ip.y][this._ip.x];
			...
			if (this._reading !== null) {
				...
				if (FishProgram.LITERALS.includes(c)) {
					...
					// The 'put' command may have changed the dimensions of the grid
					if (c === "p") {
						this._normalizeGrid();
					}
					...
					switch (c) {
						...
						default:
							// Invalid instruction
							throw new Error(`Invalid instruction '${c} (${c.charCodeAt(0)})`);
					}
				}
			}
		} catch (err) {
			...
			// Be a little bit kind and log the actual error to the console
